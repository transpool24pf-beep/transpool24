...
export default function AdminReportsPage() {
  ...
  if (loading) {
    return (
      <div className="rounded-2xl bg-white p-10 shadow-lg">
        <p className="text-[#0d2137]/70">جاري التحميل… / Loading…</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-[#0d2137]">تقارير سريعة / Quick reports</h1>
      <p className="text-sm text-[#0d2137]/70">
        أرقام مجمّعة من جدول الطلبات — يمكن توسيعها لاحقًا (لكل سائق، زمن الاستجابة، إلخ).
      </p>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border-2 border-[#0d2137]/10 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-[#0d2137]/50">إجمالي الطلبات</p>
        ...
        <div className="rounded-2xl border-2 border-emerald-200 bg-emerald-50/50 p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-emerald-800/70">إيراد (تقريبي)</p>
          <p className="mt-1 text-3xl font-bold text-emerald-900">€ {data.revenueEur}</p>
          <p className="mt-1 text-xs text-emerald-800/70">باستثناء draft / cancelled في المجموع</p>
        </div>
        <div className="rounded-2xl border-2 border-blue-200 bg-blue-50/50 p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-blue-800/70">مع سائق</p>
        ...
        <div className="rounded-2xl border-2 border-amber-200 bg-amber-50/50 p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-amber-900/70">معدل الإلغاء</p>
          <p className="mt-1 text-3xl font-bold text-amber-900">{data.cancelRatePercent}%</p>
          <p className="mt-1 text-xs text-amber-900/60">ملغاة: {data.cancelledCount}</p>
        </div>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border-2 border-[#0d2137]/10 bg-white p-6 shadow-lg">
          <h2 className="mb-4 text-lg font-semibold text-[#0d2137]">حسب الحالة logistics_status</h2>
        ...
        <div className="rounded-2xl border-2 border-[#0d2137]/10 bg-white p-6 shadow-lg">
          <h2 className="mb-4 text-lg font-semibold text-[#0d2137]">ملخص التسليم</h2>
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between">
              <dt className="text-[#0d2137]/60">تم التسليم</dt>
              <dd className="font-semibold text-green-800">{data.deliveredCount}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-[#0d2137]/60">ملغى</dt>
