...
/**
 * Website CMS: upload driver testimonial photo to Supabase Storage (public URL).
 */
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `homepage-drivers/${randomUUID()}-${safeName}.${ext}`;
    ...
    if (upErr) {
      console.error("[website/drivers/upload]", upErr);
      return NextResponse.json(
        { error: upErr.message || "Upload failed (check Storage bucket policies)." },
        { status: 500 }
      );
    }
    ...
    console.error("[website/drivers/upload]", e);
